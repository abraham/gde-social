...
exports.update_statuses = functions.pubsub
  .topic('fifteen-minute-tick')
  .onPublish(getNewStatuses);
exports.new_status = functions.pubsub
  .topic('new_status')
  .onPublish(getNewStatuses);

async function getNewStatuses(
  _message: functions.pubsub.Message,
  _context: functions.EventContext,
) {
  const latestId = await latestStatusId();
  const tweets = await twitter.getListTweets(200, latestId);
  console.log(`Got ${tweets.length} tweets`);
  return Promise.all(tweets.map(setStatus)).catch((error) =>
    console.error(new Error(`ERROR saving all, ${error}`)),
  );
}
...
function setStatus(tweet: Status) {
  return db
    .collection('statuses')
    .doc(tweet.id_str)
    .set(buildStatus(tweet))
    .catch((error) =>
      console.error(new Error(`ERROR saving ${tweet.id_str}, ${error}`)),
    );
}
...
function render(
  ...
  const oldestCreatedAt =
    statuses.length > 0 ? statuses[statuses.length - 1].createdAt : '';
